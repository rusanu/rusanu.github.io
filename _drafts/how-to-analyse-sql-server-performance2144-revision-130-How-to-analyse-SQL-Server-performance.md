...
date: 2014-02-24T05:56:22+00:00
author: remus
layout: revision
guid: http://rusanu.com/2014/02/24/2144-revision-130/
permalink: /2014/02/24/2144-revision-130/
---
...
EXECUTE sp_user_counter1 @value;
      ...
      <p>
        Developers are familiar with performance analysis using sample profiling, with tools like <a href="http://en.wikipedia.org/wiki/VTune">VTune</a>, <a href="http://msdn.microsoft.com/en-us/library/z9z62c29.aspx">F1</a> or <a href="http://msdn.microsoft.com/en-us/library/windows/hardware/hh448170.aspx">WPA</a>. The sample profiling technique consist of periodically taking a sample of each stack in the profiled process, up to several hundred times per second. The process is non-invasive, it does not require a specially instrumented process, any process can be profiled. It is also fairly lightweight, taking several hundred samples per second hash no measurable impact on the performance of the process being sampled. The trick is interpreting the observations. Using the <a href="http://support.microsoft.com/kb/311503">Microsoft public symbols server</a> you can resolve the collected samples to actual function names in the source code. With some common sense, one can make an educated guess what exactly was the SQL Server doing during the period being sampled. This technique makes sense only if you are seeing significant CPU usage. If the performance is bad and you see slow queries but low CPU usage it means the queries are waiting and you should focus instead on wait info analysis, sample profiling will reveal nothing useful.
      </p>
