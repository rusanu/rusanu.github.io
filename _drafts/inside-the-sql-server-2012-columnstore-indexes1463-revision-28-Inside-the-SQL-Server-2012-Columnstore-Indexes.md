...
date: 2012-05-17T03:03:31+00:00
author: remus
layout: revision
guid: http://rusanu.com/2012/05/17/1463-revision-28/
permalink: /2012/05/17/1463-revision-28/
---
...
Enter the new batch mode operators. In batch mode things work the same as in row mode, but instead of calling a function that returns one row at a time, it calls a function that returns many rows at a time: a batch. The benefits of doing so are not obvious for non-programmers, but believe you me, this can result in speed improvements of 2 and even 3 orders of magnitude.

The only problem with batch mode execution is that it requires everybody to play the same tune. Row mode operators cannot interact with batch mode operators nor the other way around. For best performance a query plan has to run in batch mode from top to bottom. But since not all operations were implemented in batch mode by the time SQL Server 2012 RTM shipped, sometimes there will be restrictions that prevent batch mode execution. The Query Optimizer can resort if it has too to an conversion operator from batch mode to row mode and at least can create plans that have sub trees in batch mode even if the upper part of the tree is in row mode and even this mixed mode can yield significant performance improvements, provided that most data processing still occurs in batch mode (eg. aggregation occurs in batch mode and the result of aggregation is consumed in row mode). For a lengthier discussion of how to achieve batch mode processing in some edgier Eric Hanson has some very useful tricks at <a href="http://social.technet.microsoft.com/wiki/contents/articles/4995.sql-server-columnstore-performance-tuning.aspx#Ensuring_use_of_the_Fast_Batch_Mode_of_Query_Execution" target="_blank">Ensuring Use of the Fast Batch Mode of Query Execution</a>.