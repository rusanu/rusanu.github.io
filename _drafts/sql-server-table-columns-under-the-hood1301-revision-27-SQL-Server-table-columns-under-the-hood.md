...
date: 2011-10-08T00:28:54+00:00
author: remus
layout: revision
guid: http://rusanu.com/2011/10/08/1301-revision-27/
permalink: /2011/10/08/1301-revision-27/
---
...
&lt;p></code>  
    ...
    By now, our 4 column table has actually 12 columns in the physical storage format, 8 dropped columns and 4 usable columns. The 212 bytes of fixed space size reserved originally for the records has actually grown to 312 bytes now because we decreased and then increased back a fixed column size. The variable length columns that now store the first\_name and last\_name are stored in the record _after_ these 312 reserved bytes for fixed columns that are now dropped. Since the records always consume all the reserved fixed size, this is quite wasteful. How do we reclaim it? Rebuild the table:
