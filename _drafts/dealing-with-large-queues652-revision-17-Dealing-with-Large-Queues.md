...
date: 2010-03-02T21:45:21+00:00
author: remus
layout: revision
guid: http://rusanu.com/2010/03/02/652-revision-17/
permalink: /2010/03/02/652-revision-17/
---
...
the problem with Service Broker queues is that they are not normal data tables: they are queues! Every single row is inserted and then deleted as fast as possible. No record is ever read twice. No record stays around. The queues are constantly growing, and constantly shrinking. On machines that are entirely dedicated to Service Broker processing it is not unusual to have all CPU and all I/O resources of the server dedicated to enqueuing and dequeuing messages into one single queue. In other words INSERT then quickly DELETE one row at a time, as fast as the I/O subsystem permits it. The Ghost Cleanup better keep up, as _every_ enqueued message is deleted, and _every_ deleted row is a ghosted record to be cleanup up. All of the sudden