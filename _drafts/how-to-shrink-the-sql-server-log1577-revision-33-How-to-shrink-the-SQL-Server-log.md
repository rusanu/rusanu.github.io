...
date: 2012-07-26T05:03:43+00:00
author: remus
layout: revision
guid: http://rusanu.com/2012/07/26/1577-revision-33/
permalink: /2012/07/26/1577-revision-33/
---
...
In this state the LDF file contains 3 empty VLFs and only a small active log portion. However, the physical LDF file cannot be reduced in size, any attempt to shrink the file will fail. As a general rule a file (any file) can only be shrunk (reduce in size) by removing data at the end of the file (basically by reducing the file length). Is not possible to &#8216;delete&#8217; from the beginning of a file or from the middle of a file. Unlike data MDF files, the system cannot afford to move records around in order to free space at the end of the file, remember that LSNs are basically an offset inside the file. The active portion of the log cannot be moved, because there all those records in the active portion of the log would suddenly become invalid. Therefore certain conditions must be met in order to be able to shrink (reduce the size) of an LDF log file:

<p class="callout float-right">
  Only the last VLF in a log LDF file can be removed, when is not in use
</p>

  * Only entire VLFs can be removed from the file.
  * A VLF must be 100% free (no active log records) to be eligible for removal
  * There must not be any active/in use VLFs between the current VLF and the end of file

One can rephrase the above as a statement that &#8220;Only the last VLF in a log LDF file can be removed when is not in use&#8221; and then apply the same recursively until 1) a non-free VLF is at the end of the LDF file or 2) the desired LDF file size was reached. 
